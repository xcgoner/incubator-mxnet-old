...
struct PreLAMBParam : public dmlc::Parameter<PreLAMBParam> {
  ...
  DMLC_DECLARE_PARAMETER(PreLAMBParam) {
    ...
    // .set_default(false)
    // .describe("Whether to use bias correction, in the latest version of the lamb,"
...
struct MPPreLAMBKernel {
  ...
  MSHADOW_XINLINE static void Map(int i, float* out_data, float* mean_data,
    ...
    mean_data[i] = mean;
    var_data[i] = var;
    // the latest version does not have bias correction
    float g = mean / (mshadow_op::square_root::Map(var) + param_epsilon) + param_wd * w;
    // g is already fp32, do not cast
    // KERNEL_ASSIGN(out_data[i], req, g);
    out_data[i] = g;
    // this kernel only partially implement LAMB, the output is g, not weight
...
#endif  // MXNET_OPERATOR_CONTRIB_LAMB_INL_H_
